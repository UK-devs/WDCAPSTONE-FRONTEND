...
export class DashboardComponent {
  ...
  onViewAllIntakeRecords(){
   ...
   ngOnInit(){
    ...
    this.monthYearString = `${this.currentDate.toLocaleString('default',{month: 'long'})} ${this.currentDate.getFullYear()}`;
    ...
    const ctx = document.getElementById('barChartOverallReport') as HTMLCanvasElement;  
   ...
   onViewLineGraph(){
    ...
    let ChartGraph = new Chart(ctx_line_graph_projects,{
      type: 'line',
      data: {
        labels: ['Jan', 'Feb', 'Mar','Apr','May','June','Jul','Aug','Sep','Oct','Nov','Dec'], // Your x-axis labels
    ...
    this.departmentNames.forEach((item, index)=>{
      ...
      this.data_line = [
        ...
        { x: 1, y: this.countProjectsDepartment(item,"7",current_year_string) },
        { x: 2, y: this.countProjectsDepartment(item,"7",current_year_string) },
        { x: 3, y: this.countProjectsDepartment(item,"7",current_year_string) },
        { x: 4, y: this.countProjectsDepartment(item,"7",current_year_string)},
        { x: 5, y: this.countProjectsDepartment(item,"7",current_year_string)},
        { x: 6, y: this.countProjectsDepartment(item,"7",current_year_string) },
        { x: 7, y: this.countProjectsDepartment(item,"7",current_year_string) },
        { x: 8, y: this.countProjectsDepartment(item,"8",current_year_string) },
        { x: 9, y: this.countProjectsDepartment(item,"9",current_year_string) },
        { x: 10, y: this.countProjectsDepartment(item,"10",current_year_string) },
        { x: 11, y: this.countProjectsDepartment(item,"11",current_year_string) },
        { x: 12, y: this.countProjectsDepartment(item,"12",current_year_string) }
   ...
   countProjectsDepartment(dept_item:string, month_string:string, year_string: string): number {
  ...
  onViewAllProposalRecords(){
    ...
    this.inputService.getProposalService(this.router.url).subscribe((data:any) => {
      ...
      this.proposalDetailsDisplay =  this.proposalDetailsDisplay.filter((p_item_filter:any)=>{

        return p_item_filter.proposal_root_id === '' || p_item_filter.proposal_root_id === null || p_item_filter.proposal_root_id === undefined || p_item_filter.proposal_root_id === ' -- ';

      });

      if(this.proposalDetails){

        this.proposalDetailsFilterByCurrentUser = this.proposalDetails.filter((item:any)=>{

          return item.user_id.includes(this.userId);
  
        });

        this.proposalDetailsFilterByCurrentUser = this.proposalDetailsFilterByCurrentUser.filter((item:any)=>{

          return item.proposal_root_id === "" || item.proposal_root_id === " -- " || item.proposal_root_id === undefined || item.proposal_root_id == null;
      ...
      if(this.proposalDetailsDisplay){
        ...
        this.proposal_done = this.proposalDetailsDisplay.filter((item:any)=>{

          return item.status.includes("2");
  
        });

        this.proposal_approved = this.proposalDetailsDisplay.filter((item:any)=>{
      ...
      const ctx_dps = document.getElementById('pieChart-progress-report') as HTMLCanvasElement;
      ...
      if(this.proposalDetailsFilterByCurrentUser){
        this.proposalDetailsFilterByCurrentUser = this.sortByDate(this.proposalDetailsFilterByCurrentUser,"createdAt", true);
      }
  ...
  onViewAllStudentProposal(){
    ...
    this.proposalIntakeForm = this.inputService.getIntakeFormService(this.router.url).subscribe((data:any)=>{
  ...
  onViewAllStudentProposalByUser(user_id_student:string){

    

  } // onViewAllStudentProposalByUser()
